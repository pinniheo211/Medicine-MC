...
import { actionCreateBrand, actionGetBrand } from 'store/reducers/brand';
...
export default function DialogCreateBrand({ open, setOpen }) {
  ...
  const handleNew = (data) => {
    dispatch(actionCreateBrand(data)).then((res) => {
        ...
        <DialogContent dividers>
          <form onSubmit={handleSubmit(handleNew)}>
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <TextField {...field} fullWidth error={Boolean(errors.title)} helperText={errors.title?.message || ''} label="Brand name" />
              )}
            />
