...
import { actionEditUser, actionGetAllUser } from 'store/reducers/auth';
...
export default function DialogFormUpdate({ open, setOpen, id }) {
  ...
  const { data: dataUser } = useSelector((state) => state.auth.getUserId);
  const handleClose = () => {
    setOpen(false);
  };
  ...
  const handleNew = (data) => {
    ...
    dispatch(actionEditUser(dataForm)).then((res) => {
  ...
  useEffect(() => {
    if (dataUser) {
      setValue('firstname', dataUser?.user?.firstname);
      setValue('lastname', dataUser?.user?.lastname);
      setValue('mobile', dataUser?.user?.mobile);
