...
export default function DialogFormUpdate({ open, setOpen, id }) {
  ...
  const { data: dataDesCategory } = useSelector((state) => state.category.getDesCategory);
  const handleClose = () => {
    setOpen(false);
  };
  ...
  const handleNew = (data) => {
    ...
    dispatch(actionUpdateCategory(dataForm)).then((res) => {
  ...
  useEffect(() => {
    if (dataDesCategory) {
      setValue('title', dataDesCategory?.category?.title);
    }
  }, []);
